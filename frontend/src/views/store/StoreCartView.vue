...
<template>
  ...
  >
    <div class="store-cart page-with-close">
      <PageCloseButton :fallback="{ name: 'storefront' }" />

      <header class="store-cart__header">
        <h1>{{ t('store.storefront.cart') }}</h1>
        <button type="button" class="btn btn-secondary" @click="reload">
          {{ t('store.common.refresh') }}
        </button>
      </header>

      <p v-if="!canPayWallet" class="store-cart__warn">{{ t('store.storefront.walletOnly') }}</p>
      <p v-if="error" class="store-cart__error">{{ error }}</p>
      <p v-else-if="!items.length" class="store-cart__empty">
      ...
      <template v-else>
        ...
        <p v-if="mixedPay" class="store-cart__error">{{ t('store.storefront.mixedPay') }}</p>
        ...
        <div class="store-cart__summary">
          ...
          <p class="store-cart__hint">{{ t('store.storefront.cartPayHint') }}</p>
        </div>
...
.store-cart__error { color: #b42318; }
.store-cart__warn { opacity: 0.85; }
.btn {
  display: inline-flex;
  align-items: center;
