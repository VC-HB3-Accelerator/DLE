<!--
  ...
  Вкладка: справочная матрица доступа ИИ-агента по ролям
  (ранее /settings/ai/agent-access).
-->

<template>
  <div class="roles-tab ai-agent-tab">
    ...
    <div v-if="selectedRole" class="panel role-card">
      ...
      <div class="caps-card">
        ...
        <p class="caps-hint">{{ t('settings.security.roles.aiChatCapsHint') }}</p>
        <div class="caps-grid">
          <div class="caps-item">
...
const selectedRole = computed(
  () => roles.value.find((role) => role.role === selectedRoleKey.value) || roles.value[0] || null
);
...
async function loadChatCapsMatrix() {
  try {
    ...
    console.warn('[UserRolesAiAgentTab] chat caps fallback:', error?.message || error);
  }
  return {};
}
...
async function loadAgentAccess() {
  ...
  try {
    ...
    console.error('[UserRolesAiAgentTab] load failed:', error);
...
.role-card,
.editor-card {
  margin: 0 0 var(--spacing-xl, 1.5rem);
}
...
.matrix-table tbody tr.is-active {
  background: color-mix(in srgb, var(--color-primary, #1a365d) 8%, white);
}
