[English](../legal.en/CONTRIBUTING.md) | **Русский**

# Руководство для участников разработки репозитория

Это правила **вклада в исходный код**. Они не делают участника авторизованным
 контрибьютором страны и не дают права продавать лицензии.

**Авторизованный контрибьютор** (совместное локальное юридическое лицо,
 продажа лицензий, модификация для клиентов) — отдельный модельный договор:
 [CONTRIBUTOR_LICENSE.md](CONTRIBUTOR_LICENSE.md).

## Лицензирование

**Разработка кода (этот файл):** при внесении изменений в репозиторий вы
 соглашаетесь с **Proprietary Software License**; права на вклад передаются
 автору проекта (см. ниже).

**Коммерческая деятельность (продажа лицензий, модификация для клиентов):**
 регулируется отдельно — [CONTRIBUTOR_LICENSE.md](CONTRIBUTOR_LICENSE.md).
 Без принятия CONTRIBUTOR_LICENSE продажа лицензий и модификация кода для
 третьих лиц **не допускаются**. Вклад в репозиторий это соглашение не заменяет.

## Авторские права

Все, кто вносит код в репозиторий, должны:
- Указывать свое имя в коммитах
- Соглашаться с передачей прав на код автору проекта
- Не нарушать авторские права третьих лиц

## Процесс внесения изменений

### 1. Подготовка
- Создайте fork репозитория
- Клонируйте ваш fork локально
- Создайте feature branch для ваших изменений

### 2. Разработка
- Следуйте стандартам кодирования проекта
- Добавляйте комментарии к коду на русском языке
- Тестируйте ваши изменения

### 3. Коммиты
- Используйте понятные сообщения коммитов
- Указывайте свое имя в коммитах
- Разбивайте большие изменения на логические части

### 4. Pull Request
- Создайте Pull Request с описанием изменений
- Укажите, какие проблемы решает ваш код
- Дождитесь review от автора проекта

## Стандарты кода

### Именование
- Файлы и папки: kebab-case
- Переменные и функции: camelCase
- Константы: UPPER_SNAKE_CASE
- Классы: PascalCase

### Комментарии
```javascript
/**
 * @copyright 2024-2026 Тарабанов Александр Викторович
 * @license Proprietary
 * @author [Ваше имя] <ваш-email@example.com>
 */
```

### Git сообщения
```
feat: добавить новую функцию авторизации
fix: исправить ошибку в валидации данных
docs: обновить документацию API
style: форматирование кода
refactor: рефакторинг модуля пользователей
test: добавить тесты для нового функционала
```

## Контакты

### По вопросам участия в проекте:
- **Email:** info@hb3-accelerator.com
- **GitHub Issues:** [Создать issue](https://github.com/VC-HB3-Accelerator/DLE/issues)

### Автор проекта:
**Тарабанов Александр Викторович**  
Email: info@hb3-accelerator.com

---

**Спасибо за ваш вклад в развитие проекта DLE!** 