...
11. [Соответствие требованиям по персональным данным](#соответствие-требованиям-по-персональным-данным)
...
Шаблон DLE даёт **технические и организационные средства** для обезличивания, шифрования, обработки и хранения персональных данных. Конкретные требования задаёт **применимый регулятор** в стране деятельности оператора экземпляра (GDPR, 152‑ФЗ, PIPL и аналоги — по юрисдикции). Юридическая полнота локального compliance остаётся на стороне оператора: шаблоны документов адаптируются под местное право.

### Размещение экземпляра в стране деятельности

Распространённое требование регуляторов по защите данных — **локализация**: обработка и хранение персональных данных на инфраструктуре **в пределах страны** (или иной территории, которую задаёт применимое право), а не у иностранного SaaS-вендора.

DLE рассчитан на такой контур: экземпляр устанавливается на **собственной инфраструктуре оператора** — сервер, VPS или локальное устройство **в юрисдикции деятельности**. Данные и ключ шифрования остаются под контролем оператора; правообладатель шаблона не принимает ПДн на свою сторону как обязательный облачный хостинг.

Оператор выбирает площадку размещения так, чтобы она соответствовала правилам локализации (и при необходимости — ограничениям трансграничной передачи) в своей стране.

Ниже — остальные механизмы, заложенные в продукт.

### 1. Подпись кошелька (SIWE) и согласие

Аутентификация — **Sign-In with Ethereum** (EIP-4361). В statement сообщения явно фиксируется ознакомление с документами из `Resources` и согласие на обработку персональных данных (`shared/siweStatements.json`).

В `Resources` попадают URL опубликованных consent-документов экземпляра. Подпись кошельком — криптографическое подтверждение согласия без передачи приватного ключа на сервер.

Nonce и идентификатор кошелька в таблице `nonces` хранятся через PostgreSQL `encrypt_text` (см. ниже). Журнал согласий — `consent_logs` (API: `/consent/documents`, `/consent/grant`, `/consent/revoke`).

### 2. Пакет юридических документов

UI: `/content/templates` (шаблоны) и `/content/published` (опубликованные страницы). Seed: `backend/scripts/seed/legalTemplatesSeed.js`.

**Публичные** (для субъектов данных):

| Документ | Назначение |
| --- | --- |
| Политика в отношении обработки персональных данных | Публичная политика обработки ПДн |
| Политика конфиденциальности | Политика конфиденциальности сервиса |
| Согласие на обработку персональных данных | Пользовательское согласие |
| Согласие на использование файлов cookie | Cookie по категориям |
| Согласие на трансграничную передачу ПДн | Трансграничная передача |
| Согласие на обработку биометрических ПДн | Биометрия (при необходимости) |
| Права субъектов ПДн и отзыв согласия | Права субъекта и отзыв |

**Внутренние** (доступ `view_legal_docs`): приказ и ДИ ответственного, положение об обработке и защите, регламенты обращений и исполнения запросов, политика хранения и уничтожения, разграничение доступа, перечень допущенных лиц и НДА, шаблон DPA, реестр операций, ИСПДн, модель угроз и план безопасности, реагирование на инциденты, обучение, уведомление регулятора, процедуры трансграничной передачи, согласие законного представителя, политика cookie и сторонних сервисов.

Оператор заполняет плейсхолдеры (`{{company_name}}`, `{{privacy_email}}` и др.), публикует нужные документы и привязывает их к SIWE / consent-flow.

### 3. PostgreSQL: шифрование и хранение

Чувствительные поля шифруются на уровне БД функциями `encrypt_text` / `decrypt_text` (`pgcrypto`, AES-CBC). Ключ — у оператора (`ENCRYPTION_KEY` или файл ключа экземпляра); данные остаются на **инфраструктуре оператора в выбранной юрисдикции** (см. [размещение экземпляра](#размещение-экземпляра-в-стране-деятельности)).

Типичные зашифрованные категории:

| Контур | Примеры |
| --- | --- |
| Профиль / CRM | `users.first_name_encrypted`, `last_name_encrypted` |
| Идентичности | `user_identities` — email, telegram, wallet (`*_encrypted`) |
| Auth | `nonces` — адрес и nonce |
| Сообщения | содержимое и связанные поля каналов |
| Пользовательские таблицы | имена, ячейки (`user_tables` / `user_cell_values`) |

Обезличивание и минимизация поддерживаются моделью данных (раздельные identity-провайдеры, шифрование PII at rest) и правами доступа; сроки хранения и уничтожение оператор фиксирует в опубликованной политике и внутренних регламентах.

### 4. CRM и контакты

Контур CRM (`/crm`, `/contacts-list`): контакты — записи `users` + `user_identities` с расшифровкой на стороне сервиса при наличии ключа и прав. Удаление контакта каскадно снимает связанные данные пользователя (в т.ч. гостевые сообщения — по политике удаления в API).

Обработка ПДн клиентов и сотрудников ведётся **на инфраструктуре оператора в стране (юрисдикции) деятельности**, без обязательной передачи вендору SaaS.

### Границы ответственности

| Слой | DLE (шаблон) | Оператор экземпляра |
| --- | --- | --- |
| Размещение в стране деятельности (локализация ПДн) | Установка on‑prem / на своём хосте | ✅ Выбор площадки в юрисдикции |
| Шифрование at rest, SIWE, журнал согласий, пакет шаблонов | ✅ | Настройка ключа, публикация docs |
| Выбор и адаптация текстов под местное право | Шаблоны-заготовки | ✅ Обязательно |
| Уведомление регулятора, DPA с процессорами, сроки хранения | Шаблоны | ✅ По юрисдикции |
| Хостинг и доступ к ключу шифрования | Инструкция | ✅ Контроль оператора |
