...
```
┌──────────────────────────────────────────────────────┐
│            Локальная модель Ollama (qwen2.5:1.5b)      │
...
Векторизация запроса (Ollama mxbai-embed-large → вектор [768])
       ↓
RAG-поиск по подключённым таблицам (pgvector)
       ↓
...
|-----------|------------|------------|
| LLM | Ollama (qwen2.5:1.5b или любая другая) | Генерация ответов, диалог |
| Embedding | mxbai-embed-large | Векторизация текста для поиска |
| Векторная БД | pgvector (PostgreSQL) | Семантический поиск |
...
|-------|----------|-------------------|
| Семантический | Поиск по смыслу через pgvector | Когда важна точность понимания |
