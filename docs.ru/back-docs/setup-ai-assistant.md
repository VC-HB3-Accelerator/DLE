...
2. Выберите модель:
   - **qwen2.5:1.5b** (рекомендуется) — для русского языка, ~1.0 GB
...
1. В поле **"Модель (LLM)"** выберите установленную модель:
   - `qwen2.5:1.5b` (рекомендуется для русского)
...
1. **LLM-модель**: Выберите `qwen2.5:1.5b (ollama)`
...
> - **Qwen2.5:1.5b**: Базовый контекст = **32,768 токенов** (~24,000 русских слов)
...
2. Используйте модель `qwen2.5:1.5b` вместо `llama2:7b`
