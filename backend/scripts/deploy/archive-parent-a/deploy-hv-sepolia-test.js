 ...
 * Canonical contour: MultiTest (`deploy-3-testnets-with-bridges.js`).
 * Snapshot: `scripts/contracts-data/archive/hv-sepolia-deploy.json`
 * Audit: `data-room/stage-a/AUDIT-DEPLOYMENTS-RETIRE-Parent-A.ru.md`
 *
...
async function main() {
  ...
  console.log('Deploying Treasury + Bridge + HV...');
  const treasury = await Treasury.deploy(dleAAddr, SEPOLIA, wallet.address);
  await treasury.waitForDeployment();
  const treasuryAddr = await treasury.getAddress();

  const Bridge = await ethers.getContractFactory('TreasuryBridge', wallet);
  const bridge = await Bridge.deploy(dleAAddr, treasuryAddr);
  await bridge.waitForDeployment();
  const bridgeAddr = await bridge.getAddress();
  await (await treasury.setFundsBridge(bridgeAddr)).wait();
  console.log('TreasuryBridge', bridgeAddr, 'moduleBridge', await treasury.moduleBridge());
  ...
  const out = {
    ...
    explorers: {
      ...
      treasuryBridge: `https://sepolia.etherscan.io/address/${bridgeAddr}`,
  ...
  const outPath = path.join(__dirname, '../contracts-data/archive/hv-sepolia-deploy.json');
...
async function mainGuarded() {
  if (process.env.ALLOW_DEPRECATED_PARENT_A_DEPLOY !== '1') {
    throw new Error(
      'archive-parent-a/deploy-hv-sepolia-test.js is DEPRECATED. '
      + 'Use MultiTest (deploy-3-testnets-with-bridges.js) or set ALLOW_DEPRECATED_PARENT_A_DEPLOY=1.'
    );
  }
  return main();
}
